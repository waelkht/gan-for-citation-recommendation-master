...
def equisplit(a_dict, n):
    """
    This function proposes a splitting such that the n split have roughly the same number of items in them
    ##### INPUTS #####
    :dict          A dictionary (int/strin ==> int)
    ...
    sets = a_dict.keys()
    sizes = a_dict.values()
    avg_size = np.sum(sizes) / n
    
    previous_name = sets[0]
    total = 0
    
    for set_name, set_size in zip(sets, sizes):
        total = total+set_size
        if total > avg_size:
            print "({} - {}] : {}".format(previous_name, set_name, total)
            total = 0
            previous_name = set_name
...
def dump_history(result_name, paper_indices_name, acl_metadata_name, adj_matrix_name, paper_authors_name, history):
    ...
    for paper in acl_metadata:
        ...
        for i, next_interval in enumerate(history):
            ...
            if year < next_interval:
                ...
                if not historie.has_key(year):
                    historie[year] = 0
                historie[year] = historie[year] + np.sum(adj_matrix[idx])
                
                break
            # else: do nothing
            
    print "[INFO] Proposed divisions:"
    equisplit(historie, 10)
    print ""
    ...
    # remove the real citations
    author_citations.setdiag(_zeros)
    # remove the citations going into the future
    _coo = sp.coo_matrix(author_citations)
    for paper_idx_from, paper_index_to in zip(_coo.row, _coo.col):
        paper_from = idx_to_meta[paper_idx_from]
        paper_to = idx_to_meta[paper_index_to]
        if paper_from.year <= paper_to.year:
            author_citations[paper_idx_from, paper_index_to] = 0.
    author_citations.eliminate_zeros()
    
    # ... and combine this with the adj. matrices
    paper_history_citations = []
    for adj_matrix in adj_matrices:
        # TODO: realy elementwise?
        # np.matmul(author_citations.toarray(), adj_matrix.toarray())
        paper_history_citation = author_citations * adj_matrix # matrix mult. not elementwise
        paper_history_citations.append(paper_history_citation.tocsr().astype(np.float32))
...
#                                 raw_acl, min_year, max_year, relevant_paper_ids, 2)
...
#     dump_paper_year(processed_acl_meta, processed_paper_indices, fin_pap_year_feat)
    ...
    join_feature_matrices(fin_pap_joined_feat, fin_pap_joined_divi, fin_pap_bow_feat, fin_pap_title_feat, fin_pap_auth_feat)
#     feature_to_label(fin_pap_joined_feat, adjacency_matrix, label_joined)
#     dump_history(history_matrices, processed_paper_indices, processed_acl_meta, adjacency_matrix, fin_pap_auth_feat, [1993, 1998, 2002, 2004, 2006, 2008, 2010, 2012, max_year])
#     dump_history(history_matrices, processed_paper_indices, processed_acl_meta, adjacency_matrix, fin_pap_auth_feat, [max_year-10, max_year-5, max_year])
