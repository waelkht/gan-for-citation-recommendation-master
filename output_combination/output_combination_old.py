...
def train_model(train_eval_sim, train_eval_hist, train_eval_adj_sort, num_iterations):
    ...
    # create the weights
    weights = []
    examples = [0.1, 1.0, 0, 0, 0.1]
    for i in range(num_weight):
        weights.append(tf.Variable(examples[i], dtype=tf.float32)) # 0.1 is just a random value
...
def run(result_path, history_path, other_features, model, tetd_path):
    ...
    weights = train_model(train_eval_sim, train_eval_other, train_eval_adj_sort, 150)
    ...
    # weights = [0.9905455, 0.05846505, 0.032400697, 0.027249703]
    get_recall(weights)
...
if __name__ == '__main__':
    ...
    run(result_path, history_path, [], model, tetd_path)
